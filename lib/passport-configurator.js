...
PassportConfigurator.prototype.configureProvider = function(name, options) {
  ...
  var loginCallback = options.loginCallback || function(req, done) {
    return function(err, user, identity, token, userCreated) {
      var authInfo = {
        identity: identity,
      };
      if (token) {
        authInfo.accessToken = token;
        authInfo.accessToken.userCreated = userCreated;
      }
          ...
          req.logIn(user, function(err) {
            if (err) {
              if (!!options.json) {
                var error = new Error();
                if (err.oauthError) {
                  error.status = err.oauthError.statusCode;
                  error.statusCode = err.oauthError.statusCode;
                  error.message = err.message;
                  error.name = err.name;
                  res.statusCode =  err.oauthError.statusCode;
                } else {
